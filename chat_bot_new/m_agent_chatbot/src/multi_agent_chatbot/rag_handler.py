...
def extract_text_from_image(image_data) -> str:
    ...
    try:
        ...
        if not isinstance(image_data, Image.Image):
            try:
                # 이미지 데이터가 PDF에서 추출된 경우, 추가 처리가 필요할 수 있음
                ImageFile.LOAD_TRUNCATED_IMAGES = True  # 잘린 이미지도 로드 시도
                ...
                if image is None:
                    # 이미지 포맷이 인식되지 않는 경우, raw 데이터로 처리 시도
                    try:
                        # 데이터 크기로부터 적절한 이미지 크기 추정
                        data_size = len(decoded_data)
                        if data_size % 3 == 0:  # RGB
                            pixels = data_size // 3
                            width = height = int(pixels ** 0.5)
                            if width * height * 3 == data_size:
                                image = Image.frombytes('RGB', (width, height), decoded_data)
                        elif data_size % 4 == 0:  # RGBA
                            pixels = data_size // 4
                            width = height = int(pixels ** 0.5)
                            if width * height * 4 == data_size:
                                image = Image.frombytes('RGBA', (width, height), decoded_data)
                    except Exception as e:
                        logger.warning(f"Failed to create image from raw data: {str(e)}")
                        return ""
                
                if image is None:
                    return ""
                
                # PDF에서 추출된 이미지는 다양한 색상 모드를 가질 수 있음
        ...
        try:
            ...
            # 이미지 향상
            # 그레이스케일로 변환하여 OCR 성능 향상
            image = image.convert('L')
            
            # 이미지 향상
            image = ImageOps.autocontrast(image)  # 자동 대비 조정
            
            # 선명도 향상
            enhancer = ImageEnhance.Sharpness(image)
            image = enhancer.enhance(1.5)
            
            # 노이즈 제거를 위한 중간값 필터 적용
        ...
        try:
            ...
            logger.error("Tesseract is not properly installed. Please install Tesseract OCR and make sure it's in your PATH.")
            logger.error("Windows: Download from https://github.com/UB-Mannheim/tesseract/wiki")
            logger.error("Set TESSDATA_PREFIX environment variable to the 'tessdata' directory")
            return ""
...
def process_and_embed_pdf(temp_file_path: str, original_filename: str = None) -> bool:
    """
    PDF 파일을 처리하고, Vector DB에 저장합니다.
    이미지가 포함된 PDF도 처리할 수 있도록 개선되었습니다.
    """
    try:
        print(f"Starting PDF processing: {temp_file_path}")
        
        # 원본 파일명이 없으면 임시 파일의 basename 사용
        ...
        for existing_id, info in pdf_index.items():
            if os.path.exists(info["path"]):
                if calculate_file_hash(info["path"]) == file_hash:
                    print(f"Duplicate file detected. Reusing existing data for {original_filename}")
                    # 중복 파일의 메타데이터가 실패 상태인 경우 processed로 업데이트
        ...
        # 영구 저장소로 파일 복사
        permanent_file_path = copy_to_permanent_storage(temp_file_path, original_filename)
        print(f"Copied to permanent storage: {permanent_file_path}")
        ...
        try:
            def process_pdf():
                ...
                if not all_texts:
                    try:
                        with pdfplumber.open(permanent_file_path) as pdf:
                            total_pages = len(pdf.pages)
                            for i, page in enumerate(pdf.pages):
                                # PDF 페이지 박스 처리
                                try:
                                    # CropBox가 없는 경우 MediaBox 사용
                                    if not page.cropbox:
                                ...
                                for image in page.images:
                                    try:
                                        ...
                                        # 여러 가지 방법으로 이미지 데이터 추출 시도
                                        if "stream" in image:
                                            image_bytes = image["stream"]
                                        elif "bytes" in image:
                                ...
                                if text.strip():
                                    all_texts.append(Document(page_content=text, metadata={"page": i + 1, "source": permanent_file_path}))
                    except Exception as e:
                ...
                # 3. PyMuPDF로 시도
                if not all_texts and fitz is not None:
                    try:
                        doc = fitz.open(permanent_file_path)
                        for i, page in enumerate(doc):
                            ...
                            for img_index, img in enumerate(image_list):
                                try:
                                    ...
                                    logger.warning(f"PyMuPDF image extraction failed on page {i + 1}: {str(e)}")
                ...
                # 4. UnstructuredPDFLoader로 마지막 시도
                if not all_texts:
                    try:
                        loader = UnstructuredPDFLoader(permanent_file_path)
                        docs = loader.load()
                        all_texts.extend(docs)
                    except Exception as e:
                        logger.warning(f"UnstructuredPDFLoader failed: {str(e)}")
            ...
            process_with_timeout(process_pdf, timeout=MAX_PROCESSING_TIME)
            
            # 성공적으로 처리됨
            pdf_metadata[pdf_id]["status"] = "processed"
            save_pdf_metadata()
            return True
            ...
            pdf_metadata[pdf_id]["error"] = error_msg
            save_pdf_metadata()
            return False
